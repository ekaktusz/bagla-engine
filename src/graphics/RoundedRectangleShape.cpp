...
namespace bgl
{
RoundedRectangleShape::RoundedRectangleShape(sf::Vector2f size, float radius, unsigned int cornerPoints) : m_size(size), m_radius(radius), m_cornerPoints(cornerPoints)
{
	update();
}
...
sf::Vector2f RoundedRectangleShape::getPoint(std::size_t index) const
{
	...
	return sf::Vector2f(m_radius * std::cos(radian) + center.x, -m_radius * std::sin(radian) + center.y);
}
